"""Complementary modalities: does receiving now beat NoShare for starved cars?"""
...
CONDS=[("NoShare","NoShare",False),("DFL-naive","DFL-Gossip",False),
       ("DFL-gated","DFL-Gossip",True),("RECD-gated","RECD",True)]
...
def summ(h):
    ...
    return dict(acc_all=float(a.mean()),
                ...
                acc_fed=float(a[~m].mean()),coverage_end=float(h["coverage"][-1]))
res={}
for label,method,gated in CONDS:
    rows=[]
    for s in SEEDS:
        cfg=dict(method=method,trace=TRACE,seed=s,rounds=ROUNDS,r_v2v=200.0,
                 cache_encoders=4,max_out=3,V=50.0,use_gat=False,dataset="fmnist",
                 phi_agg=0.15,local_steps=10,starve_frac=0.20,gated_agg=gated,
                 modality_mode="complementary",comp_noise=0.7)
...
json.dump(res,open(os.path.join(BASE,"results","diag_comp.json"),"w"),indent=1)
print("\n===== complementary (noise0.7) + starve20% + gated, R=25 =====")
print(f'{"cond":12}{"acc_all":>9}{"acc_starved":>13}{"acc_fed":>9}{"cov":>7}')
for label,_,_ in CONDS:
    r=res[label]
    print(f'{label:12}{r["acc_all"]:9.1f}{r["acc_starved"]:13.1f}{r["acc_fed"]:9.1f}{r["coverage_end"]:7.2f}')
print("saved results/diag_comp.json")
